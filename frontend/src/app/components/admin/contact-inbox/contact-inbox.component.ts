...
export class ContactInboxComponent implements OnInit {
    ...
    chargerMessages(): void {
        ...
        this.serviceMessageContact.obtenirTousLesMessages().subscribe({
            next: (messages: any[]) => {
    ...
    ouvrirMessage(message: MessageContact): void {
        ...
        const needsMarkAsRead = !message.lu || (message.reponseClient && !message.luParAdmin);
        if (needsMarkAsRead) {
            this.serviceMessageContact.marquerCommeLu(message.id).subscribe({
                next: () => {
                    const updatedMessage = { ...message, lu: true, luParAdmin: true };
    ...
    envoyerReponse(): void {
        ...
        this.serviceMessageContact.repondreAuMessage(this.selectedMessage.id, this.replyText).subscribe({
    ...
    supprimerMessage(message: MessageContact, event: Event): void {
        event.stopPropagation();
        if (confirm('Êtes-vous sûr de vouloir supprimer ce message ?')) {
            this.serviceMessageContact.supprimerMessage(message.id).subscribe({
                next: () => {
                    this.messages = (this.messages as any[]).filter((m: any) => m.id !== message.id);
    ...
    obtenirNombreNonLus(): number {
        return (this.messages as any[]).filter((m: any) => !m.lu).length;
    }
}
